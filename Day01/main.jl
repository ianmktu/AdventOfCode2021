...
begin		
	...
	lines = Array{Int64}(undef, countlines(input_file))

	open(input_file) do f
		current_index = 1
		for l in eachline(f)
        	lines[current_index] = parse(Int64, l)
			current_index += 1
    	end
	end	
end
...
begin			
	local increasing_counter = 0
	local prev_num = 0
	
	for (index, num_str) in enumerate(lines)
		if index == 1
			prev_num = num_str
			continue
		end
	...
	println("Part 1: ", increasing_counter)
	(increasing_counter)
end
...
begin	
	local increasing_sum_counter = 0
	local first_num = 0
	local second_num = 0
	local third_num = 0
	local prev_sum = 0
	...
	for (index, num_str) in enumerate(lines)
		global increasing_counter, prev_num
		...
		if index in [1,2,3]
			...
			elseif index == 2
				...
				third_num = num_str
				prev_sum = first_num + second_num + third_num
			end
			continue
		end
		...
		if prev_sum < curr_sum
	   		increasing_sum_counter += 1
		end
		
		prev_sum = curr_sum
	end	
	
	println("Part 2: ", increasing_sum_counter, "\n")
